...
class Runner:
    """RULE Ablation training and evaluation runner."""
    ...
    def _evaluate(self, test_left, test_right, last_epoch=False, save_name="", save_feature=False):
        ...
        avg_h1 = (float(acc_l2r[0]) + float(acc_r2l[0])) / 2
        avg_h5 = (float(acc_l2r[1]) + float(acc_r2l[1])) / 2
        avg_h10 = (float(acc_l2r[2]) + float(acc_r2l[2])) / 2
        avg_mrr = (mrr_l2r + mrr_r2l) / 2

        logger.info("KG1 -> KG2")
        logger.info(f"Hits@1 : {acc_l2r[0]:.6f}")
        logger.info(f"Hits@5 : {acc_l2r[1]:.6f}")
        logger.info(f"Hits@10: {acc_l2r[2]:.6f}")
        logger.info(f"MRR    : {mrr_l2r:.6f}")

        logger.info("KG2 -> KG1")
        logger.info(f"Hits@1 : {acc_r2l[0]:.6f}")
        logger.info(f"Hits@5 : {acc_r2l[1]:.6f}")
        logger.info(f"Hits@10: {acc_r2l[2]:.6f}")
        logger.info(f"MRR    : {mrr_r2l:.6f}")

        logger.info("Average")
        logger.info(f"Hits@1 : {avg_h1:.6f}")
        logger.info(f"Hits@5 : {avg_h5:.6f}")
        logger.info(f"Hits@10: {avg_h10:.6f}")
        logger.info(f"MRR    : {avg_mrr:.6f}")
