...
output_file = os.path.join(
    ...
    f"_ratio{ratio_tag}_seed{args.seed}.json"
)
...
def get_complete_description(ent_id, side, entity_info, attributes, relations, all_names):
    ...
    if os.path.exists(image_path):
        ...
        visual_supplement = (
            "No image is available, so no complementary visual information is used."
        )
...
def build_alignment_prompt(
    ...
    return (
        ...
        "a complete ranking with no ties. "
        + name_rule
        + " Do not use the original retrieval similarities. Give greater weight to precise and consistent facts than "
        "to generic visual resemblance.\n\n"
        f"[SOURCE ENTITY]\n{source_description}\n\n"
        "[CANDIDATE ENTITIES]\n"
        + "\n\n".join(content)
