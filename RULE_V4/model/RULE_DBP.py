...
class RULE(nn.Module):
    ...
    def __init__(self, kgs, args, train_set, test_set, logger):
        ...
        # 仅作为 availability mask；训练过程中不再被 uncertainty / consensus 动态修改
        ...
        self.multimodal_encoder = RULE_Encoder(args=self.args, ent_num=kgs["ent_num"], img_feature_dim=img_dim, char_feature_dim=char_dim, attr_input_dim=att_dim, triple_size=self.triple_size, node_size=self.node_size, rel_size=self.rel_size)

        self.modality_num = 4 + (2 if self.args.use_surface else 0)
    ...
    def joint_emb_generat(self):
        _, emb_dict = self._normalized_emb_dict()
